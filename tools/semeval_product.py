"""Title-aware ST1 product experiment; earlier frozen implementations are unchanged."""
import argparse
...
PUBLIC = ROOT / "evaluation/semeval-st1-v3"
OUTPUT = ROOT / "outputs/semeval-st1-v3"
FIELDS, INVALID = original.FIELDS, original.INVALID
VARIANTS = ("title", "title-weighted")
...
def target_hash(row):
    return digest({"title": row["title"], "text": row["text"]})
...
class TrainingRetriever:
    def __init__(self, train, config):
        ...
        self.title_vectorizer = TfidfVectorizer(ngram_range=(1, 2), sublinear_tf=True, strip_accents="unicode", max_features=40000)
        self.title_matrix = self.title_vectorizer.fit_transform([r["title"] for r in train])
        self.normalized = [normalized_text(r["text"]) for r in train]

    def examples(self, row):
        # Only the allowed title/text fields influence queries. Both indices fit training only.
        text = row["text"]
        body_scores = (self.matrix @ self.vectorizer.transform([text]).T).toarray().ravel()
        title_scores = (self.title_matrix @ self.title_vectorizer.transform([row["title"]]).T).toarray().ravel()
        weight = self.config["title_weight"]
        scores = weight * title_scores + (1 - weight) * body_scores
        ...
        for index in ranked:
            ...
            result.append({"training_id": row[""], "similarity": float(scores[index]), "title": row["title"], "text": excerpt,
                           "annotated_product": row["product"], "annotated_hazard": row["hazard"],
...
def request_for(row, protocol, examples):
    return {"modelId": protocol["model_id"], "system": [{"text": protocol["system_prompt"]}],
            "messages": [{"role": "user", "content": [{"text": json.dumps({
                "labelled_training_examples": examples, "title_to_classify": row["title"], "report_to_classify": row["text"]}, ensure_ascii=False)}]}],
...
def prepare():
    ...
    instructions = (
        ...
        "Use the target title to identify the recalled product; the full report supplies hazard and context. "
        "The body may omit the product named in the title or include unrelated page boilerplate. "
        "Classify the affected food product, not its packaging or an allergen ingredient. "
        "Prefer the specific category covering that product rather than other food product / mixed when a specific category applies. "
    ...
    common = {
        ...
        "input": "Target title plus full target text without truncation: changed input setup relative to v1/v2. Examples include training titles, text excerpts, fine product/hazard annotations and categories. No target annotations or other metadata enter inference.",
        ...
        "test_exposure": "Both earlier test results were inspected; product-error examples used for this design came from validation only. New test run is a further regression comparison after exposure, not fresh held-out evidence. Title-plus-text is an input adaptation. Underlying model contamination unknown.",
        ...
        "code_hashes": {"tools/semeval_product.py": original.sha(Path(__file__)), "tools/semeval_eval.py": original.sha(Path(original.__file__)), "uv.lock": original.sha(ROOT / "uv.lock")},
    }
    incumbent_path = ROOT / "evaluation/semeval-st1-v2/validation-retrieval/results.json"
    incumbent = json.loads(incumbent_path.read_text(encoding="utf-8"))
    plan = {"frozen_at": original.now(), "candidates": list(VARIANTS),
            "incumbent_validation_score": incumbent["official_st1_score"], "incumbent_report_sha256": original.sha(incumbent_path),
            "selection_rule": "Highest unrounded official validation score; tie: fewer invalid/error outputs, then title. Both run once on all 565 validation rows. Only if winner strictly beats the fixed v2 incumbent, freeze it for one test regression run; otherwise retain incumbent without another test run.",
    ...
    for variant in VARIANTS:
        original.write_json(PUBLIC / f"validation-{variant}-protocol.json", {
            **common, "variant": variant, "split": "validation", "data_file": "incidents_valid.csv",
            "retrieval": {**common["retrieval"], "title_weight": 0.0 if variant == "title" else 0.7},
...
def select():
    ...
    def rank(variant):
        ...
        return (-1 if value is None else value, -result["invalid_outputs"]-result["error_rows"], variant == "title")
    winner = max(VARIANTS, key=rank)
    plan = json.loads((PUBLIC / "plan.json").read_text(encoding="utf-8"))
    improved = results[winner]["official_st1_score"] is not None and results[winner]["official_st1_score"] > plan["incumbent_validation_score"]
