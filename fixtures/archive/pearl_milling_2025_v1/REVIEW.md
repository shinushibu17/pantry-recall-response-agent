...
This is one **development** recall and eight synthetic inventory scenarios.
...
- The table contains **one** product row: Pearl Milling Company **Original**
  ...
  Code Date & Manufacturing Code `BBD SEP 13 25 P`. These conditions are ANDed.
...
- `lot_code` intentionally holds the **entire printed composite code**.
...
| --- | --- | --- |
| affected | Correct identity, size, UPC, full code, all six labels checked | AFFECTED; hold recommended; action open |
| missing_lot | Correct product/UPC, complete code absent | NEEDS_EVIDENCE; inspect four boxes at A2 |
| excluded_lot | Correct product; all three boxes say SEP 14, outside SEP 13 scope | Propose NOT_AFFECTED_BY_THIS_RECALL for review |
...
| mixed_lots | Only one of nine labels has an excluded code | NEEDS_EVIDENCE; inspect/split the bin |
...
`fixture.lock.json` freezes hashes before the first matcher run. Source spans
